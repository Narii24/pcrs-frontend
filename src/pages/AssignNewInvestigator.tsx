...
const AssignNewInvestigator = () => {
  ...
  const handleDeployment = async () => {
    if (!selectedCaseId || !selectedUserId) return alert(t(language, 'missingSelection'));
    ...
    try {
      ...
      alert(t(language, 'unitAssignedSuccessfully'));
  ...
  return (
    <div className="p-10 bg-[#06080f] min-h-screen text-white">
      <h2 className="text-5xl font-black uppercase italic mb-10">{t(language, 'assignInvestigatorTitle')}</h2>
      <div className="max-w-2xl bg-[#0f111a] p-10 rounded-[3rem] border border-white/5">
        <div className="mb-6">
          <label className="text-[10px] font-black uppercase text-gray-500 mb-2 block">{t(language, 'caseDossierLabel')}</label>
          <select 
            className="w-full bg-black border border-white/10 p-4 rounded-xl text-white outline-none"
            value={selectedCaseId}
            onChange={(e) => setSelectedCaseId(e.target.value)}
          >
            <option value="">{t(language, 'selectCasePlaceholder')}</option>
            {openCases.map(c => <option key={c.caseId} value={c.caseId}>{c.caseId} - {c.title}</option>)}
          </select>
        </div>

        <div className="mb-8">
          <label className="text-[10px] font-black uppercase text-gray-500 mb-2 block">{t(language, 'investigatorLabel')}</label>
          <select 
            className="w-full bg-black border border-white/10 p-4 rounded-xl text-white outline-none"
            value={selectedUserId}
            onChange={(e) => setSelectedUserId(e.target.value)}
          >
            <option value="">{t(language, 'selectPersonnelPlaceholder')}</option>
        ...
        >
          {isSyncing ? t(language, 'syncing') : t(language, 'confirmAssignment')}
        </button>
      </div>
    </div>
  );
